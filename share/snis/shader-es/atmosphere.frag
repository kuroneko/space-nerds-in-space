...
uniform lowp float u_Alpha;		// Relative alpha, 0.0 - 1.0.
uniform lowp float u_atmosphere_brightness; // 0.0 - 1.0, default 0.5  Brightness of atmosphere

varying highp vec3 v_Position;       // Interpolated position for this fragment.
varying lowp vec3 v_Color;          // This is the color from the vertex shader interpolated across the triangle per fragment
varying mediump vec3 v_Normal;         // Interpolated normal for this fragment.
...
void main()
{
	...
	mediump vec3 lightVector = u_LightPos - v_Position;
	...
	mediump float eyedot = max(0.0, (1.0 - abs(dot(normalize(v_Normal), normalize(v_Position)))));
	mediump float eyedot2 = eyedot * eyedot;
	...
	// of scattered light.
	lowp float lightdot = dot(normalize(v_Normal), normalize(lightVector));

	// Subtract some blue-green (opposite of orange)  as we get near the
	// terminator to get a sunset effect.
	lowp float oranginess = pow(1.0 * abs(1.0 - abs(lightdot)), 12.0);
	lowp vec4 notorange = 0.5 * vec4(0.0, 0.4, 1.0, 0.7 * u_Alpha);

	// As atmosphere thins out at edges, scattered light must fall off.
	// Here is a super duper crude way to do that:
	mediump float attenuation = min((1.0 - eyedot), 0.25);
	attenuation = 15.0 * attenuation * attenuation;

	lowp float ring_shadow = 1.0;

#if defined(USE_ANNULUS_SHADOW)
	mediump float direct = 3.75 * attenuation * lightdot *eyedot2 * 0.7;
	...
	lowp vec4 fragcolor = 1.25 * vec4(v_Color * attenuation * u_atmosphere_brightness * lightdot * eyedot2 * 0.7,
