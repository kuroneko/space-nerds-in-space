...
attribute highp vec3 tvertex0;
attribute highp vec3 tvertex1;
attribute highp vec3 tvertex2;
attribute mediump vec3 edge_mask;
...
void main(void)
{
	...
	highp vec4 pos  = ModelViewProjectionMatrix * vec4(position, 1.0);
	highp vec4 pos0 = ModelViewProjectionMatrix * vec4(tvertex0, 1.0);
	highp vec4 pos1 = ModelViewProjectionMatrix * vec4(tvertex1, 1.0);
	highp vec4 pos2 = ModelViewProjectionMatrix * vec4(tvertex2, 1.0);

	// Get the screen coordinates of all vertices
	mediump vec3 p  = vec3(0.5 * Viewport * (pos.xy / pos.w), 0.0);
	mediump vec3 p0 = vec3(0.5 * Viewport * (pos0.xy / pos0.w), 0.0);
	mediump vec3 p1 = vec3(0.5 * Viewport * (pos1.xy / pos1.w), 0.0);
	mediump vec3 p2 = vec3(0.5 * Viewport * (pos2.xy / pos2.w), 0.0);

	// Get the vectors representing the edges of the current
	// triangle primitive. 'vN' is the edge opposite vertex N.
	mediump vec3 v0 = p2 - p1;
	mediump vec3 v1 = p2 - p0;
	mediump vec3 v2 = p1 - p0;
	...
	mediump float d0 = length(cross(p - p1, v0)) / length(v0);
